package br.com.puc.tcc.web.service;
...
import br.com.puc.tcc.web.controller.util.OAuthRequestWrapper;
import br.com.puc.tcc.web.controller.util.SegurancaAPIThreadLocal;
import br.com.puc.tcc.web.domain.SegurancaAPI;
import br.com.puc.tcc.web.domain.Usuario;
import br.com.puc.tcc.web.exception.TokenCriadoException;
import br.com.puc.tcc.web.exception.TokenExpiradoException;
import br.com.puc.tcc.web.exception.TokenInvalidoException;
import br.com.puc.tcc.web.exception.UsuarioOuSenhaInvalidaException;
import br.com.puc.tcc.web.repository.SegurancaRepository;
import br.com.puc.tcc.web.repository.UsuarioRepository;
import br.com.puc.tcc.web.util.DateUtils;
import br.com.puc.tcc.web.util.FormatadorUtil;
...
public class SegurancaService {
	...
	/**
	 * Metodo usado para verificar a validade do Token da requisicao. Invocado
	 * no interceptor antes do acesso ao recurso.
	 *
	 * @param request
	...
	public synchronized void verificaValidadeTokenAdicionandoNoContexto(HttpServletRequest request)
			throws TokenExpiradoException, TokenInvalidoException {
		try {
	...
	/**
	 * Metodo chamado pelo WS para guardar o Token temporario da empresa a ser
	 * retornado para o cliente da requisicao.
	 *
	 * @param chave
	 * @param token
	 */
	private synchronized void atualizarToken(Usuario usuario, String token, Date proximaDataExpiracao)
			throws TokenInvalidoException, TokenExpiradoException, TokenCriadoException {
		if (usuario == null) {
	...
	private Date retornarProximaDataExpiracao() {
		...
		return DateUtils.retornaData(dia + "/" + mes + "/" + ano + " 23:59:59",
	...
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	public OAuthResponse logarOAuth(HttpServletRequest request, Map<String, String> form) {
		try {
			...
			String senha = form.get("password");
			...
			return OAuthASResponse.tokenResponse(HttpServletResponse.SC_OK).setAccessToken(accessToken)
					.setExpiresIn(this.transformarProximaDataExpiracaoEmSegundos(new Date(), proximaDataExpiracao))
					.setParam("nome", usuario.getNome()).setParam("login", usuario.getLogin())
					// .setParam("usuario", usuario);
					.setParam("perfil", usuario.getPerfil().getNome()).buildJSONMessage();
	...
	public OAuthResponse retornarErroOAuth(int errorCode, String error, Exception e) {
		try {
			...
			return OAuthASResponse.errorResponse(errorCode)
					.setError(error + (descricao != null ? " - " + descricao : "")).setErrorDescription(descricao)
					.buildJSONMessage();
		} catch (OAuthSystemException ex) {
			throw new RuntimeException(ex);
		}
